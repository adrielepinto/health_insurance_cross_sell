...
Working with data will always be a challenge, given the power and modeling possibilities that models bring. Classifying customers who would possibly buy new car insurance gave me knowledge from data collection via SQL to putting the model into production. Modeling the data, I learned that Boruta's algorithm is not always the best for selecting the features to perform the model, depending on the business problem. This project gave me a deeper understanding of tree-based algorithms. In reality, Random Forest - RF is an algorithm created in an attempt to solve one of the Decision Tree problems. Seeing that the probability of error from just one tree would be much greater, a forest was created, which denotes an 'electoral vote', where the class with the most votes of all trees wins the 'election' (class to be defined). Also, I learned a little more about the model's metrics, such as the Lyft curve, which shows how much better the model is than selecting customers randomly, and the gain curve that evaluates the model's efficiency in relation to the database. . It taught me that it is possible to reduce business costs, since this metric tells me how much of the database it will actually be necessary to work on. Putting the model into production always seems like something out of this world, but working on this project, I realized that a model in production on Google Sheets is efficient and not as difficult as it seems. I can see that the result of this model can improve a lot, if there is a new collection of data that better explains the phenomenon.
