...
Working with data will always be a challenge, given the power and modeling possibilities that the models bring.
Classifying customers who would possibly buy new car insurance gave me knowledge from data collection via SQL to putting the model into production. One of the lessons I learned is that the Boruta algorithm is not always the best for features select to perform the model, depending on the business problem. Regarding the classification algorithm, this problem gave me a deeper understanding of tree-based algorithms. In reality, Random Forest - RF is an algorithm created in an attempt to solve one of the Decision Tree - DS problems. Seeing that the probability of error from just one tree (DS) would be much greater, a forest was created, which denotes an 'electoral vote', where the class with the most votes of all trees wins the 'election' (class to be defined ).
I also learned a little more about the model's metrics such as the Lyft curve, which shows how much better the model is than selecting customers randomly, and also the gain curve that evaluates the model's efficiency in relation to the database. This shows that, it is possible to reduce costs for the company, since this metric tells me the amount of the database that will actually be necessary to work on to reach 100% of those interested cutomers, thus being able to eliminate the percentage of customers who are not interested. Putting the model into production I realized that a model in production on Google Sheets is efficient, practical and easy to access for managers to use to make decisions. It is still possible to improve the result of this model and it could improve a lot if there is a new collection of data that better explains the phenomenon.
